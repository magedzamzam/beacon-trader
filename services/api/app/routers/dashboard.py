...
async def dashboard_summary(db: AsyncSession = Depends(get_db)):
    ...
    total_pl = float((await db.execute(select(func.coalesce(
        func.sum(Trade.realized_pl), 0)))).scalar() or 0)
    open_legs = (await db.execute(select(func.count(Leg.id)).where(
        Leg.status.in_(("open", "working", "pending"))))).scalar() or 0
    closed = (await db.execute(select(func.count(Leg.id)).where(Leg.status == "closed"))).scalar() or 0
    wins = (await db.execute(select(func.count(Leg.id)).where(Leg.outcome == "tp_hit"))).scalar() or 0
