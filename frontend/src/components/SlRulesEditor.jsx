...
/* rules: [{trigger:{type,index|points}, action:{type:'move_sl_to', target:'entry'|'number', value?}}] */
  ...
  return (
    <div className="space-y-2">
      ...
      {list.map((r, i) => {
        ...
        const setAct = a => update(i, { ...r, action: a });
        return (
          <div key={i} className="flex flex-wrap items-end gap-2 border border-edge rounded-lg p-2 bg-panel2">
            ...
            <Select value={trig.type} onChange={e => setTrig(
              ...
              <option value="price_move">price moves</option>
            </Select>
            {trig.type === "tp_hit" ? (
              <div className="w-20"><NumberInput value={trig.index ?? 1}
                onChange={e => setTrig({ ...trig, index: +e.target.value })} /></div>
            ) : (
              <div className="w-24"><NumberInput value={trig.points ?? 3}
                onChange={e => setTrig({ ...trig, points: +e.target.value })} /></div>
            )}
            <div className="text-xs text-muted">move SL to</div>
            <Select value={act.target} onChange={e => setAct(
              e.target.value === "entry" ? { type: "move_sl_to", target: "entry" }
                                         : { type: "move_sl_to", target: "number", value: act.value || 0 })}>
              <option value="entry">entry</option>
              <option value="number">number</option>
            </Select>
            {act.target === "number" && (
              <div className="w-28"><NumberInput value={act.value ?? 0}
