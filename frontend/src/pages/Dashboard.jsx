  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPI label="Realized P&L" value={money(kpi.total_pl)} tone={tone(kpi.total_pl)} sub="closed legs" />
        <KPI label="Win rate" value={`${kpi.win_rate}%`} tone="beacon" sub="TP hits / closed" />
        <KPI label="Open positions" value={kpi.open_legs} sub={`${kpi.open_trades} trades`} />
        <KPI label="Total trades" value={kpi.total_trades} sub="all time" />
      </div>
      ...
      <Card>
        ...
        {recent.length === 0 ? <Empty>No trades yet. Send a signal to the manual desk to see the fanout.</Empty> : (
          <table className="w-full">
            ...
            <tbody>
              {recent.map(t => (
                <tr key={t.id} className="border-b border-edge/60">
                  <Td mono>{t.id}</Td>
                  <Td>{t.symbol}</Td>
                  <Td><Badge tone={t.direction === "BUY" ? "long" : "short"}>{t.direction}</Badge></Td>
                  <Td><Badge tone={t.status === "closed" ? "muted" : "beacon"}>{t.status}</Badge></Td>
